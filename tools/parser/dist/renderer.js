...
class S3Renderer {
    ...
    renderToHtml(document, options = {}) {
        ...
        const blocksHtml = document.blocks.map(block => this.renderBlockToHtml(block, defaultOptions)).join('\n');
        const metadataHtml = this.renderMetadataToHtml(document, defaultOptions);
        if (defaultOptions.wrapInDocument) {
            return this.wrapInHtmlDocument(metadataHtml + blocksHtml, defaultOptions);
        }
     ...
     */
    renderBlockToHtml(block, options = {}) {
        const defaultOptions = {
            theme: 'light',
            syntaxHighlighting: true,
            showLineNumbers: false,
    ...
    exportToMarkdown(document, options = {}) {
        ...
        if (defaultOptions.includeMetadata && defaultOptions.frontMatter && document.metadata) {
            ...
            if (document.metadata.created)
                markdown += `date: ${document.metadata.created}\n`;
            if (document.metadata.description)
                markdown += `description: "${document.metadata.description}"\n`;
            if (document.metadata.tags)
                markdown += `tags: [${document.metadata.tags.map(t => `"${t}"`).join(', ')}]\n`;
            markdown += '---\n\n';
        }
        // Add title
     ...
     */
    renderStatsToHtml(document) {
        const blockCount = document.blocks.length;
        const languages = new Set(document.blocks.map(block => block.language === 'multi' && typeof block.code === 'object'
    ...
    renderCodeToHtml(block, options) {
    ...
    renderMultiLanguageCodeToHtml(code, defaultLanguage, options = {}) {
    ...
    renderBlockToMarkdown(block, options) {
        let markdown = '';
        ...
        if (options.includeCodeBlocks) {
            ...
            if (block.language === 'multi' && typeof block.code === 'object') {
                ...
                markdown += this.formatCodeBlock(block.code, block.language, options.codeBlockStyle || 'fenced');
            }
        }
        return markdown;
    }
    formatCodeBlock(code, language, style) {
        if (style === 'fenced') {
            return `\`\`\`${language}\n${code}\n\`\`\``;
        }
        else {
    ...
    renderMetadataToHtml(document, options) {
    ...
    wrapInHtmlDocument(content, options) {
