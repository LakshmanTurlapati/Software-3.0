...
window.addEventListener('message', event => {
    ...
    switch (message.type) {
        ...
        case 'generation-started':
            setAIButtonGenerating(true);
            break;
        case 'generation-progress':
            // Could show progress message if desired
            console.log('[AI] Generation progress:', message.message);
            break;
        case 'generation-complete':
            setAIButtonGenerating(false);
            // Refresh code display
            const codeTextarea = document.getElementById('code-textarea');
            const codeDisplay = document.getElementById('code-display');
            if (codeTextarea && codeDisplay) {
                updateCodeDisplay(codeTextarea.value);
            }
            break;
        case 'generation-error':
            setAIButtonGenerating(false);
            console.error('[AI] Generation error:', message.error);
            break;
        case 'generation-cancelled':
            setAIButtonGenerating(false);
            break;
        case 'void-environment-status':
            // Always show AI button regardless of environment
            const aiButton = document.getElementById('ai-button');
            if (aiButton) {
                // Always show the button
                aiButton.style.display = 'inline-flex';
                document.body.classList.add('in-void');
                console.log('[AI] AI features enabled for all environments');
            }
            break;
    }
});
...
function initializeEditor() {
    ...
    try {
        ...
        // Add event listener for AI button (Void only)
        const aiButton = document.getElementById('ai-button');
        if (aiButton) {
            // Remove any existing listeners first
            ...
            newAIButton.addEventListener('click', function(e) {
                console.log('[BUTTON] AI button clicked!');
                e.preventDefault();
                e.stopPropagation();
                handleAIButtonClick();
            });
            console.log('[INIT] AI button click listener added successfully');
            
            // Check if we're in Void and show the button
            checkForVoidAndShowAI();
        }
        ...
        // Initialize requirements panel references (for Python files)
        ...
        const requirementsToggleIcon = document.getElementById('requirements-toggle-icon');
        ...
        // Set up requirements panel handlers (for Python files)
        if (requirementsHeader) {
            requirementsHeader.addEventListener('click', function(e) {
                if (e.target.closest('#toggle-requirements')) {
                    e.stopPropagation();
                }
        ...
        if (toggleRequirementsBtn) {
            toggleRequirementsBtn.addEventListener('click', function(e) {
                e.stopPropagation();
                toggleRequirementsPanel();
            });
        }
        ...
        if (requirementsTextarea) {
            requirementsTextarea.addEventListener('input', function() {
                markDirty();
                // Auto-save requirements on change
                if (vscode) {
                    vscode.postMessage({
                        type: 'change',
                        edit: {
                            type: 'requirements',
                            content: requirementsTextarea.value
                        }
                    });
                }
            });
        ...
        // Helper function to toggle requirements panel
        function toggleRequirementsPanel() {
            if (requirementsContent) {
                requirementsContent.classList.toggle('collapsed');
                if (requirementsToggleIcon) {
                    requirementsToggleIcon.className = requirementsContent.classList.contains('collapsed') 
                        ? 'fas fa-chevron-down' 
                        : 'fas fa-chevron-up';
                }
            }
        ...
        if (instructionsTextarea) {
            ...
            instructionsTextarea.addEventListener('input', handleInstructionChange);
...
function handlePlayButtonClick() {
    ...
    // Get requirements if this is a Python file
    let requirements = '';
    const requirementsTextarea = document.getElementById('requirements-textarea');
    if (requirementsTextarea && (language === 'python' || language === 'py')) {
        requirements = requirementsTextarea.value || '';
    }
...
function setAIButtonGenerating(generating) {
    const aiButton = document.getElementById('ai-button');
    const aiIcon = document.getElementById('ai-icon');
    
    if (!aiButton || !aiIcon) return;
    ...
    if (generating) {
        ...
        aiIcon.parentElement.innerHTML = '<span class="ai-loader"></span>';
        aiButton.title = 'Generating code...';
    } else {
        aiButton.classList.remove('generating');
        aiButton.disabled = false;
        const newIcon = document.createElement('i');
        newIcon.className = 'fas fa-hammer';
        newIcon.id = 'ai-icon';
        aiButton.innerHTML = '';
        aiButton.appendChild(newIcon);
        
        // Re-add the badge if in Void
        const badge = document.createElement('span');
        badge.className = 'void-badge';
        badge.textContent = 'VOID';
        aiButton.appendChild(badge);
        
        aiButton.title = 'Generate code with AI';
    }
}
...
function handleAIButtonClick() {
    ...
    // Get instructions
    const instructionsTextarea = document.getElementById('instructions-textarea');
    const instructions = instructionsTextarea ? instructionsTextarea.value : '';
    
    if (!instructions || instructions.trim() === '') {
        console.log('[AI] No instructions to generate from');
        return;
    }
    
    // Check if code is empty or instructions changed
    const codeTextarea = document.getElementById('code-textarea');
    const code = codeTextarea ? codeTextarea.value : '';
    ...
    if (vscode) {
        ...
        console.error('[AI] VS Code API not available');
    }
}

function checkForVoidAndShowAI() {
    // Check if we're in Void by looking for certain indicators
    // This will be set by the extension
    const aiButton = document.getElementById('ai-button');
    if (aiButton) {
        // The extension will handle visibility based on Void detection
        // For now, we'll make it visible if VS Code API is available
        if (vscode) {
            vscode.postMessage({ type: 'check-void-environment' });
        }
    }
}

// Auto-generate on instruction changes (with debouncing)
let instructionChangeTimeout = null;
function handleInstructionChange() {
    const instructionsTextarea = document.getElementById('instructions-textarea');
    const codeTextarea = document.getElementById('code-textarea');
    
    if (!instructionsTextarea || !codeTextarea) return;
    
    const currentInstructions = instructionsTextarea.value;
    const currentCode = codeTextarea.value;
    
    // Clear any existing timeout
    if (instructionChangeTimeout) {
        clearTimeout(instructionChangeTimeout);
    }
    
    // Only auto-generate if:
    // 1. Instructions have changed
    // 2. Code is empty or very short
    // 3. We're in Void (AI button is visible)
    const aiButton = document.getElementById('ai-button');
    if (aiButton && aiButton.style.display !== 'none' &&
        currentInstructions !== lastInstructions &&
        currentInstructions.trim() !== '' &&
        (currentCode.trim() === '' || currentCode.trim().length < 50)) {
        
        instructionChangeTimeout = setTimeout(() => {
            console.log('[AI] Auto-generating code due to instruction change');
            handleAIButtonClick();
            lastInstructions = currentInstructions;
