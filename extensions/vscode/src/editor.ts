...
import { isRunningInVoid } from './ai/voidDetection';
...
interface S3Document {
  ...
  code: string;
  language?: string;
  requirements?: string; // Optional requirements for Python files
}
 ...
 */
interface S3Edit {
  type: 'instructions' | 'code' | 'language' | 'requirements';
  content: string;
  timestamp: number;
}
...
class S3CustomDocument extends vscode.Disposable implements vscode.CustomDocument {
  ...
  makeEdit(edit: S3Edit): void {
    ...
    } else if (edit.type === 'requirements') {
      s3Doc.requirements = edit.content;
    }
...
export class S3DocumentEditor implements vscode.CustomEditorProvider<S3CustomDocument> {
  ...
  private generationInProgress: Map<string, boolean> = new Map();
  private lastInstructions: Map<string, string> = new Map();
  ...
  constructor(private readonly context: vscode.ExtensionContext) {
    ...
    this.aiService.initialize(context);
  }
      ...
      switch (e.type) {
        ...
        case 'execute':
          ...
          if (e.code && e.language) {
            try {
              // Pass the S3 file path and requirements for Python files
              const s3FilePath = document.uri.fsPath;
              const requirements = e.requirements || '';
              await this.executionHandler.execute(e.code, e.language, webviewPanel, s3FilePath, requirements);
            } catch (error: any) {
        ...
        case 'generate-code':
          ...
          console.log('[S3Editor] Generate code request received');
          const docUri = document.uri.toString();
          
          // Check if already generating for this document
          if (this.generationInProgress.get(docUri)) {
            console.log('[S3Editor] Generation already in progress');
            return;
          }
          
          // Check if AI service is available
          if (!this.aiService.isAvailable()) {
            // AI available in all environments - just need API key
            webviewPanel.webview.postMessage({
              type: 'generation-error',
              error: 'AI service not configured. Please provide an API key.'
            });
            // Re-initialize to prompt for key
            await this.aiService.initialize(this.context);
            return;
          }
          
          // Start generation
          this.generationInProgress.set(docUri, true);
          webviewPanel.webview.postMessage({ type: 'generation-started' });
          ...
          try {
            ...
            const codeEdit: S3Edit = {
              type: 'code',
              content: result.code,
              timestamp: Date.now()
            };
            document.makeEdit(codeEdit);
            
            // Update language if detected
            if (result.language) {
              const langEdit: S3Edit = {
                type: 'language',
                content: result.language,
                timestamp: Date.now()
              };
              document.makeEdit(langEdit);
            }
            
            // Update requirements for Python
            if (result.requirements && result.language === 'python') {
              const reqEdit: S3Edit = {
                type: 'requirements',
                content: result.requirements,
                timestamp: Date.now()
              };
              document.makeEdit(reqEdit);
            }
            ...
            // Save last instructions for change detection
            this.lastInstructions.set(docUri, instructions);
            
            webviewPanel.webview.postMessage({ type: 'generation-complete' });
            vscode.window.showInformationMessage('Code generated successfully!');
            
          } catch (error: any) {
            console.error('[S3Editor] Generation error:', error);
            webviewPanel.webview.postMessage({
              type: 'generation-error',
              error: error.message || 'Failed to generate code'
            });
          } finally {
            this.generationInProgress.set(docUri, false);
          }
          return;
        case 'cancel-generation':
          // Handle cancel generation request
          console.log('[S3Editor] Cancel generation request received');
          this.aiService.cancelGeneration();
          this.generationInProgress.set(document.uri.toString(), false);
          webviewPanel.webview.postMessage({ type: 'generation-cancelled' });
          return;
        case 'check-void-environment':
          // Always report as if in compatible environment
          console.log('[S3Editor] Environment check - AI enabled for all environments');
          webviewPanel.webview.postMessage({ 
            type: 'void-environment-status',
            isVoid: true  // Always true to enable AI button
          });
          return;
      }
    });
  ...
  private async getHtmlForWebview(webview: vscode.Webview, document: S3CustomDocument): Promise<string> {
    ...
    try {
      ...
      if (!('instructions' in s3Document) || !('code' in s3Document)) {
