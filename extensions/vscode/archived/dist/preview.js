...
class S3PreviewPanel {
    static createOrShow(extensionUri, documentUri) {
        ...
        const panel = vscode.window.createWebviewPanel(S3PreviewPanel.viewType, 'Software3 Preview', column || vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [
        ...
        S3PreviewPanel.currentPanels.set(documentUri.toString(), new S3PreviewPanel(panel, extensionUri, documentUri));
    }
    static revive(panel, extensionUri, documentUri) {
        S3PreviewPanel.currentPanels.set(documentUri.toString(), new S3PreviewPanel(panel, extensionUri, documentUri));
    }
    constructor(panel, extensionUri, documentUri) {
        ...
        const saveWatcher = vscode.workspace.onDidSaveTextDocument((e) => {
    ...
    async _wrapInVSCodeHtml(document, webview) {
        // Get VS Code theme
...
exports.S3PreviewPanel = S3PreviewPanel;
/**
 * Track the currently active panels. Only allow a single panel per .s3 file.
 */
S3PreviewPanel.currentPanels = new Map();
S3PreviewPanel.viewType = 'software3Preview';
